        ...
        if (recentProjectsContainer && projects.length > 0) {
            recentProjectsContainer.innerHTML = projects.slice(0, 3).map(project => {
                const imageUrl = project.image_url ? 
                    (project.image_url.startsWith('http') ? project.image_url : `http://72.62.53.138:8000${project.image_url}`) : 
                    ...
                    <td><input type="checkbox"></td>
                                <td>
                                    <div class="table-project">
                                        <img src="${project.image_url ? (project.image_url.startsWith('http') ? project.image_url : `http://72.62.53.138:8000${project.image_url}`) : 'https://via.placeholder.com/40'}" alt="مشروع">
    ...
    modal.querySelector('#uploadEditProjectImage').addEventListener('click', () => {
        ...
        createImageUploadInput((url, isLocalPreview) => {
            ...
            if (editImagePreviewImg && url) {
                ...
                // إذا كانت URL نسبية، أضف السيرفر للعرض
                editImagePreviewImg.src = url.startsWith('http') ? url : (url.startsWith('/') ? `http://72.62.53.138:8000${url}` : url);
                editImagePreviewImg.style.display = 'block';
            }
            ...
            if (!isLocalPreview) {
                // url هو بالفعل مسار نسبي مثل /uploads/images/filename.jpg
                selectedImageUrl = url.startsWith('http') ? url.replace('http://72.62.53.138:8000', '') : url;
